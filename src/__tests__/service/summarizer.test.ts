...
describe('summarize — level별 길이 제한', () => {
  it('level 1 결과가 1000자 이하', async () => {
    const result = await summarize(makeLongMarkdown(), 1);
    expect(result.length).toBeLessThanOrEqual(1000);
  });

  it('level 2 결과가 2000자 이하', async () => {
    const result = await summarize(makeLongMarkdown(), 2);
    expect(result.length).toBeLessThanOrEqual(2000);
  });

  it('level 5 결과가 8000자 이하', async () => {
    const result = await summarize(makeLongMarkdown(), 5);
    expect(result.length).toBeLessThanOrEqual(8000);
  });

  it('level 1이 level 5보다 짧음', async () => {
    const md = makeLongMarkdown();
    const level1 = await summarize(md, 1);
    const level5 = await summarize(md, 5);
    expect(level1.length).toBeLessThan(level5.length);
  });

  it('level 기본값(3)은 비어있지 않음', async () => {
    const result = await summarize('# Title\nThis is a sample sentence for testing purposes.');
...
describe('summarize — 헤딩 보존', () => {
  it('섹션 헤딩 포함', async () => {
    ...
    const result = await summarize(md);
    expect(result).toContain('# Introduction');
    expect(result).toContain('## Features');
  });

  it('헤딩만 있고 본문 없는 경우 헤딩 유지', async () => {
    const md = '## Steps\n- Step one is here\n- Step two is here\n- Step three is here';
    const result = await summarize(md);
...
describe('summarize — 엣지 케이스', () => {
  ...
  it('산문 없는 헤딩만 있어도 구조 반환', async () => {
    const md = '# Title Only\n## Another Section';
    const result = await summarize(md);
    expect(result.length).toBeGreaterThan(0);
  });

  it('maxChars 초과하면 첫 페이지만 반환', async () => {
    const md = makeLongMarkdown(100);
    const result = await summarize(md, 1);
    expect(result.length).toBeLessThanOrEqual(1000);
  });

  it('짧은 마크다운은 전체 반환', async () => {
    const md = '# Title\nShort content.';
    const result = await summarize(md, 5);
    expect(result).toContain('Title');
  });

  it('level 0 → level 1로 clamp', async () => {
    const result = await summarize(makeLongMarkdown(), 0);
    expect(result.length).toBeLessThanOrEqual(1000);
  });

  it('level 6 → level 5로 clamp', async () => {
    const result = await summarize(makeLongMarkdown(), 6);
    expect(result.length).toBeLessThanOrEqual(8000);
  });

  it('중복 텍스트 문장도 올바르게 처리', async () => {
